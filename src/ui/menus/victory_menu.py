...
class VictoryMenu():
    def __init__(self,game,position):
        ...
        self.background = pg.image.load(PATH_ASSETS+"visual/ui/victory.png")
        self.background = pg.transform.scale(self.background,self.size)

        self.ui_elements_button = [
            GameToMainMenuButton(game.window,(RES_X*0.7,RES_Y*0.8),game),
            GameToNextGameButton(game.window,(RES_X*0.3,RES_Y*0.8),game),
        ]
...
class GameToNextGameButton(Button):
    def __init__(self,window,position,game):
        super().__init__(window,position)
        self.game = game
